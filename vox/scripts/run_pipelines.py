...
if __name__ == "__main__":
    ...
    parser.add_argument("ref_file", help="path to reference transcript file in nlp format")
    parser.add_argument("output_dir", help="path to output directory")
    parser.add_argument("--num_speakers", help="number of speakers")
    parser.add_argument("--whisper_model", help="valid whisper model name or path to checkpoint", default=WHISPERMODEL)
    parser.add_argument("--pipelines_to_run", help="pipelines to run", default="all", choices=["all", "1.4", "1"])
    args = parser.parse_args()

    files_to_score = []
    pipelines_to_run = args.pipelines_to_run.split(".") if args.pipelines_to_run != "all" else ["1", "2", "3", "4"]
    ...
    transcribe_result_file = (Path(output_dir) / f"{Path(audio_file).name}.json").resolve()
    files_to_score.append((1, transcribe_result_file))

    if not Path(transcribe_result_file).is_file():
        model = whisper.load_model(args.whisper_model)
        result = model.transcribe(audio_file, verbose=False, condition_on_previous_text=True, beam_size=4)
    ...
    if "2" in pipelines_to_run:
        ...
        drz_post_sr_reco_file = (Path(drz_post_sr_output_dir) / f"{Path(audio_file).name}.json").resolve()
        files_to_score.append((2, drz_post_sr_reco_file))

        if not Path(drz_post_sr_reco_file).is_file():
            result["segments"] = add_speakers_to_segments(audio_file, result["segments"], num_speakers=int(args.num_speakers))
    ...
    if "3" in pipelines_to_run:
        ...
        drz_pre_sr_reco_file = (Path(drz_pre_sr_output_dir) / f"{Path(audio_file).name}.json").resolve()
